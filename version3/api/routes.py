...
@api_bp.route('/download-pem', methods=['GET'])
def download_pem():
    pem_file_path = os.path.join(os.path.dirname(__file__), '..', 'terraform', 'linuxkey.pem')
    
    if not os.path.exists(pem_file_path):
        logging.error("PEM file not found")
        return jsonify({"status": "error", "message": "PEM file not found"}), 404
    
    try:
        return send_file(pem_file_path, as_attachment=True, download_name='linuxkey.pem')  # Use download_name for Flask >= 2.0
    except Exception as e:
        logging.error(f"Error sending PEM file: {str(e)}")
        return jsonify({"status": "error", "message": "Error sending PEM file"}), 500
